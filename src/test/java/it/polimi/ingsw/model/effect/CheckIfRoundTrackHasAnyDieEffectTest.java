...
class CheckIfRoundTrackHasAnyDieEffectTest {

    @Test
    void testPerform() {
        ToolCardMock toolCardMock = new ToolCardMock();
        toolCardMock.setToolCardMock();
        CheckIfRoundTrackHasAnyDieEffect check = new CheckIfRoundTrackHasAnyDieEffect("checkIfRoundTrackHasAnyDie");
        check.setToolCard(toolCardMock);
        check.perform(toolCardMock.getBoard());
        assertFalse(toolCardMock.isEverythingOk());
        toolCardMock.addDieToRoundTrack();
        check.perform(toolCardMock.getBoard());
    ...
    private class ToolCardMock extends it.polimi.ingsw.model.ToolCard {
        ...
        public void addDieToRoundTrack(){
            List<Die> dice1 = new ArrayList<>();
            Die die1 = new Die(SagradaColor.PURPLE);
            dice1.add(die1);
            board.getRoundTrack().addRound(dice1);
        }
